        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="mt-[100px] max-w-[1200px] mx-auto p-4">
                ...
                <h2 className="text-xl font-semibold mb-4">Thông tin đơn hàng</h2>
                    ...
                    <div>
                    <p><span className="font-medium">Khách hàng:</span>{orders?.user_id}</p>
                    <p><span className="font-medium">Email:</span> {orders?.email || "không có"}</p>
                    <p><span className="font-medium">SĐT:</span> {orders?.phone}</p>
                    </div>
                    <div>
                    <p><span className="font-medium">Địa chỉ:</span> {orders?.address}</p>
                    ...
                    {orders?.status === 1 ? (
                        ...
                        <span className="text-green-600 font-semibold">Đang soạn hàng</span>

                    ) : orders?.status === 3 ? (
                        <span className="text-green-600 font-semibold">Đang vận chuyển</span>

                    ) : (
                        <span className="text-green-600 font-semibold">Đã giao xongn</span>

                    )}
                    </p>
                    <p><span className="font-medium">Ghi chú:</span> {orders?.note}</p>
                    </div>
                </div>
                <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                    <p><span className="font-medium">Phí ship:</span> {orders?.ship_fee}</p>
                    ...
                    <p><span className="font-medium">Tổng tiền:</span> <span className="font-bold text-red-500">{orders?.total_amount}</span></p>
                </div>
                </div>
                    ) : (
                    ...
                    <tbody className="text-gray-700">
                        {products.length > 0 ? (
                                products.map((pr,index)=>(
                                    <tr className="border-t" key={`${pr.product}-${pr.color_id}-${pr.size_id}`}>
