...
import { User,ShoppingCart,Heart,Search,ClockFading,X, Bell } from 'lucide-react';
...
return(
<header className={`fixed top-0 z-10 left-0 w-full bg-white shadow transition-transform duration-300 
}`}
>
   ...
   */}
   <div>
      <ul className="flex gap-6 text-xl font-semibold font-poppins ">
         ...
         <li className="hover:cursor-pointer hover:text-gray-400 transition-colors duration-300 py-[20px]">
            <Link href="http://localhost:3000/page/Shop">
            Cửa hàng</Link>
         </li>
         ...
         <li className="hover:cursor-pointer hover:text-gray-400 transition-colors duration-300 py-[20px]">
            ...
            Cửa hàng của bạn</Link>
            ) : (
            <Link href="http://localhost:3000/sellerregistration">
            Đăng kí bán hàng</Link>
            )}
         </li>
         <li className="hover:cursor-pointer hover:text-gray-400 transition-colors duration-300 py-[20px]">
            <Link href="http://localhost:3000/page/support">
            Hỗ trợ</Link>
         </li>
      </ul>
   </div>
...
<div className="flex items-start gap-2 cursor-pointer text-gray-700 "
   onClick={() =>
   router.push(`/search/?keyword=${encodeURIComponent(item.keyword)}`)}
   >
   {/* 
 ...
 <X className="cursor-pointer size-6 p-0 m-0 text-gray-400 hover:text-red-500 transition" onClick={() =>
...
<div className="relative" ref={registerCart} >
   <ShoppingCart className="size-7 hover:cursor-pointer" onClick={()=>
   Opencart()}/>
   {cart.length > 0 ?(
...
<div className="relative">
   <Bell className="size-7 hover:cursor-pointer" onClick={() =>
   OpenNoti()}/>
   {isOpenNoti && (
      ...
      <div className="">
         ...
         <div className=" h-[400px] overflow-y-auto hide-scrollbar">
            <div className="border-b p-2 relative hover:cursor-pointer hover:bg-gray-100">
               <div>
                  <p className="p-0 m-0 text-[17px]">Đơn hàng 12345678 của bạn đang trên đường vận chuyển</p>
               ...
               <div className="w-[10px] h-[10px] bg-red-500 rounded-2xl absolute bottom-[15px] right-2">
               </div>
            </div>
      ...
      <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded-xl shadow-lg  z-10">
         <div className="p-2">
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-gray-100 cursor-pointer">
               ...
               <span className="text-[15px]" onClick={() => OpenNoti()}>Thông báo</span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-gray-100 cursor-pointer">
               <svg
                  xmlns="http://www.w3.org/2000/svg"
